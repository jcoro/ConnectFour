...
public class Connect4Slot
{
    ...
    public boolean getIsRed()
    {
     ...
     /**
     * Checks if the token in the slot matches the player's color. 
     * 
     * If the slot is not filled, this will still return false; so, this should only
     * be checked after checking getIsFilled().
     *    
     * 
     * @return true if the token in the slot matches the player's color.
     */
    public boolean isMine(boolean iAmRed)
    {
        if (iAmRed)
        {            
            return isRed;
